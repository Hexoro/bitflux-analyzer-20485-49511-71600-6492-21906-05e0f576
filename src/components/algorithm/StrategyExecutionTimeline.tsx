/**
 * Strategy Execution Timeline - Visual timeline with persistent history
 * Shows all execution runs with step-by-step details
 */
...
const TIMELINE_STORAGE_KEY = 'bsee_execution_timeline';
...
export const StrategyExecutionTimeline = ({ isExecuting = false }: StrategyExecutionTimelineProps) => {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        setRuns(parsed.map((r: any) => ({ ...r, timestamp: new Date(r.timestamp) })));
        if (parsed.length > 0) {
  ...
  const saveRuns = (newRuns: ExecutionRun[]) => {
    setRuns(newRuns);
    localStorage.setItem(TIMELINE_STORAGE_KEY, JSON.stringify(newRuns.slice(0, 50))); // Keep last 50
  };
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Select value={selectedRunId || ''} onValueChange={setSelectedRunId}>
                <SelectTrigger className="w-60 h-8 text-xs bg-background/50 border-cyan-400/30">
                  <SelectValue placeholder="Select run" />
                </SelectTrigger>
                <SelectContent>
                  {runs.map(run => (
                    <SelectItem key={run.id} value={run.id}>
                      {run.strategyName} - {run.timestamp.toLocaleString()}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <CardContent className="p-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-[10px] text-muted-foreground">
                ...
                <span>Total: {selectedRun.totalDuration.toFixed(2)}ms</span>
              </div>
            </div>
                  ...
                  {steps.map((step, index) => (
                    <div
                      ...
                      className={`relative p-3 rounded-lg border cursor-pointer transition-all ${getStepColor(step.status, index)} ${
                        index === currentStepIndex ? 'ring-1 ring-cyan-400' : ''
                      }`}
              ...
              <Card className="bg-background/30 border-cyan-400/20">
                ...
                <CardContent className="p-3 space-y-2 text-xs">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    {currentStep.cost !== undefined && (
                      <div className="p-2 bg-muted/30 rounded">
                        <div className="text-muted-foreground text-[10px]">Cost</div>
                        <div className="font-mono font-medium">{currentStep.cost}</div>
                      </div>
                    )}
